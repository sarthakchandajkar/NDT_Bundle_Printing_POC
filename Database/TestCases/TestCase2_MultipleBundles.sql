...
-- Create PO_Plan for Test Case 2 (using DO block to handle insert/update)
DO $$
DECLARE
...
BEGIN
    ...
    FROM "PO_Plan"
    WHERE "PO_No" = 'PO_TEST_MULTIPLE_BUNDLES';
...
BEGIN
    ...
    IF v_po_plan_id IS NOT NULL THEN
        ...
        FROM "M1_Slit"
        WHERE "Slit_No" = 'SLIT_TEST_MULTIPLE_BUNDLES';
        
        IF v_slit_id IS NULL THEN
            INSERT INTO "M1_Slit" ("PO_Plan_ID", "Slit_No", "Status", "Slit_NDT", "SlitMillStartTime")
            VALUES (v_po_plan_id, 'SLIT_TEST_MULTIPLE_BUNDLES', 2, 0, CURRENT_TIMESTAMP);
        ELSE
            UPDATE "M1_Slit"
